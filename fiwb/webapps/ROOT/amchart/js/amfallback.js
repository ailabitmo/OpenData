AmCharts.AmFallback = AmCharts.Class(
{
    ...
    loadAndParseData:function()
    {
        ...
        if (chartData != undefined && chartData != '')
        {
            ...
            {
                if(this.type == 'stock')
                    this.parseStockCSV(chartData, data);
                else
        ...
        {
            dataFile = this.settings.data_sets.data_set.file_name;
            
            if(this.settings.path)
            {
                dataFile = this.settings.path + dataFile;
            }
    ...
    processSerialChart:function()
    {
        ...
        //a trick to make the long label text visible at the bottom of a chart
        if(type == "column")
        {
       ...
       processStockChart:function()
    {
       ...
       if(this.data[0].out1 === undefined)
       {                                           
            var stockPanel = new AmCharts.StockPanel();
            stockPanel.addLabel(0, 100, "No data available.", "center", 12);
            chart.panels = [stockPanel];
            stockPanel.color = "#333333";
            return;
       }
       
       var settings = this.settings;
    
       chart.pathToImages = "/amchart/js/amstock/images/"; 
       chart.dataProvider = this.data;

       
//       this.setSettings(chart, settings, this.StockSettingsMap);
       ...
       // PANEL ///////////////////////////////////////////       
        ...
        this.setSettings(stockPanel, settings, this.StockSettingsMap);
        ...
        var labels = [];       
        if(this.settings.valueLabels)
            labels = this.settings.valueLabels.split(",");
        
        //graphs type//////////////////
        var graphType = this.settings.lineType;
        if(graphType == undefined)
            graphType = "step";
            
        var columnInfo = this.data[0];    
        var y = 0;
        for(property in columnInfo)
        {
            if(property.indexOf("out") != -1)
            {
                dataSet.fieldMappings.push({fromField:property, toField:property});
                
                var graph = new AmCharts.StockGraph();
                
                var graphSettings = settings.charts.chart.graphs.graph; 
                if(graphSettings instanceof Array)
                    graphSettings = settings.charts.chart.graphs.graph[0]; 
                    
                if (graphSettings)
                {
                    this.setSettings(graph, graphSettings, this.StockGraphSettingsMap);
     
                }
                        
                graph.valueField = property;
                ...
                {
                    graph.lineColor = colors[y];
                }
                if(labels[y] != undefined)
                {
                    graph.title = labels[y];
                }
        ...
        var legendSettings = settings.charts.chart.legend; 
        
        if (legendSettings)
        {
            this.setSettings(stockLegend, legendSettings, this.stockLegendMap);
        }
        ...
        stockPanel.addValueAxis(valAxis); 
        
        this.setSettings(valAxis, graphSettings, this.StockValueAxisSettingsMap); 
        
        // OTHER SETTINGS ////////////////////////////////////
        var scrollbarSettings = new AmCharts.ChartScrollbarSettings();
        
        var scrollSettings = settings.scroller; 
        
        if (scrollSettings)
        {
            this.setSettings(scrollbarSettings, scrollSettings, this.scrollbarMap);
        }
        ...
        var periodSelectorSettings = settings.period_selector; 
        
        if (periodSelectorSettings)
        {
            this.setSettings(periodSelector, periodSelectorSettings, this.PeriodSelectorMap);


        periodSelector.periods = this.parsePeriods(periodSelectorSettings);
        ...
        this.setSettings(panelsSettings, settings, this.StockSettingsMap);
        chart.panelsSettings = panelsSettings;
        

    },
    
    parsePeriods:function(periodSettings)
    {
        var periods = [];
        var periodsArray = periodSettings.periods.period;

        for(var i =0; i < periodsArray.length; i++)
        {
            var period = periodsArray[i];
            var prop = '#text';
            var obj = {
                    period: period.type,
                    count: period.count,
    ...
    parseXMLNode: function (dataNode, dataNodeName, xmlNode, level)
    {
        ...
        if (xmlNode)
        {
            ...
            if ((n === 0) && (typeof dataNode === 'object') && (dataNode[dataNodeName].length === undefined))
            {
    ...
    loadCsv: function (file, holder)
    {
        ...
        request.send();
        if(this.type == 'stock')
            this.parseStockCSV(request.responseText, holder);
        else
    ...
    parseCSV:function(data, holder)
    {
       ...
       parseStockCSV:function(data, holder)
    {
        var separator = this.settings.csv_separator;
        if(separator == undefined)
        {
    ...
    {   
        this.dataType = "xml";
        this.pathToImages = "/amchart/js/images/";
        ...
        this.commonSettingsMap = [
            ...
            {c:'balloonText',                       s:'balloonText',                    t:'{s}'},
            {c:'height',                            s:'height',                         t:'n'},
            {c:'width',                             s:'width',                          t:'n'},
            {c:'backgroundAlpha',                   s:'background.alpha',               t:'a'},
            {c:'borderColor',                       s:'background.border_color',        t:'c'},
            {c:'borderColor',                       s:'borderColor',                    t:'c'},
        ...
        this.legendMap = [
            ...
            {c:'valueTextRegular',          s:'values.text',            t:'{s}'}
        ];
        ...
        this.pieSettingsMap = [
            ...
            {c:'startEffect',                       s:'startEffect',                    t:'e'},
        ...
        this.radarSettingsMap = [
            {c:'radius',                            s:'radius',                        t:'s'},
            {c:'startDuration',                     s:'startDuration',                  t:'n'},
            {c:'startEffect',                       s:'startEffect',                    t:'e'},
            {c:'sequencedAnimation',                s:'sequencedAnimation',             t:'b'}          
        ];
        
        this.radarAxisMap = [
            {c:'radarCategoriesEnabled',            s:'axes.value.radarCategoriesEnabled',  t:'b'},
            {c:'axisAlpha',                         s:'axes.value.axisAlpha',           t:'a'},
            {c:'axisColor',                         s:'axes.value.axisColor',           t:'c'},
            {c:'axisThickness',                     s:'axes.value.axisThickness',       t:'n'},
            {c:'minimum',                           s:'axes.value.minimum',             t:'n'},
            {c:'dashLength',                        s:'axes.value.dashLength',          t:'n'},
            {c:'gridCount',                         s:'axes.value.gridCount',           t:'n'},
            {c:'gridType',                          s:'axes.value.gridType',            t:'s'},
            {c:'gridColor',                         s:'axes.value.gridColor',           t:'c'},
            {c:'gridAlpha',                         s:'axes.value.gridAlpha',           t:'a'},
            {c:'gridThickness',                     s:'axes.value.gridThickness',       t:'n'},
            {c:'axisTitleOffset',                   s:'axes.value.axisTitleOffset',     t:'n'}
                        
        ];      
        
        this.serialSettingsMap = [
            {c:'parseDate',                         s:'parseDate',                      t:'b'},
        ...
        this.categoryAxisMap = [
            {c:'labelRotation',         s:'labelRotation',                  t:'n'},         
            {c:'labelRotation',         s:'axes.{a}.labelRotation',         t:'n'},     
            ...
            {c:'axisThickness',         s:'axes.{a}.width',                 t:'n'},
            {c:'tickLength',            s:'axes.{a}.tick_length',           t:'n'},
            {c:'labelRotation',         s:'values.{a}.labelRotation',       t:'n'}
        ];
        ...
        this.graphMap = [
            ...
            {c:'customBullet',                      s:'customBullet',                   t:'s'},
        ...
        this.graphMapC = [
            ...
            {c:'balloonColor',                      s:'balloonColor',                   t:'c'}
        ];  
        ...
        this.graphMapLine = [
            ...
            {c:'lineThickness',                     s:'lineThickness',                  t:'n'},
            {c:'lineAlpha',                         s:'alpha',                          t:'a'},
            {c:'fillAlpha',                         s:'fill_alpha',                     t:'a'},
            {c:'bullet',                            s:'bullet',                         t:'s'},
            {c:'bulletSize',                        s:'bullet_size',                    t:'n'},
            {c:'bulletSize',                        s:'bulletSize',                     t:'n'},
            {c:'labelText',                         s:'data_labels',                    t:'{s}'},
            {c:'balloonText',                       s:'balloonText',                    t:'{s}'},
            {c:'balloonText',                       s:'balloon_text',                   t:'{s}'},
            {c:'urlTarget',                         s:'link_target',                    t:'s'},
        ...
        this.scrollbarMap = [
            ...
            {c:'height',                            s:'height',                         t:'n'},
            {c:'graph',                             s:'graph_data_source',              t:'s'}
        ...
        /////STOCK SETTINGS//////////////////////////
        this.StockSettingsMap = [   
            
            {c:'categoryAxesSettings.equalSpacing', s:'equal_spacing',                  t:'b'},
            {c:'margins',                           s:'margins',                        t:'n'},
            {c:'borderAlpha',                       s:'charts.chart.border_alpha',      t:'a'},
            {c:'title',                             s:'title',                          t:'s'},
            {c:'colors',                            s:'colors',                         t:'arr'},
            {c:'fillColors',                        s:'colors',                         t:'arr'},
            {c:'fillAlphas',                        s:'fillAlphas',                     t:'a'},
			{c:'drawingIconsEnabled',               s:'drawingIconsEnabled',            t:'b'},
            {c:'marginLeft',                        s:'margin_left',                    t:'n'}
            ],
        
        this.StockGraphSettingsMap = [  
            
            {c:'title',                             s:'title',                          t:'s'},
            {c:'stackable',                         s:'stackable',                      t:'b'},
            {c:'lineColor',                         s:'colors',                          t:'c'},
            {c:'lineAlpha',                         s:'line_alpha',                     t:'a'},
            {c:'lineAlpha',                         s:'alpha',                          t:'a'},
            {c:'lineThickness',                     s:'line_width',                     t:'n'},
            {c:'fillAlphas',                        s:'fill_alpha',                     t:'a'},
            ...
            {c:'bulletSize',                        s:'bullet_size',                    t:'n'},
            {c:'customBullet',                      s:'customBullet',                   t:'s'},
            {c:'pointPosition',                     s:'pointPosition',                  t:'s'},
            {c:'balloonColor',                      s:'balloon_color',                  t:'c'},
            {c:'balloonAlpha',                      s:'balloon_alpha',                  t:'a'},
            {c:'balloonTextColor',                  s:'balloon_text_color',             t:'c'},
            {c:'balloonText',                       s:'balloon_text',                   t:'{s}'},
        ...
        this.StockValueAxisSettingsMap = [
        
        {c:'position',                   s:'axis',                        t:'s'},
        {c:'minimum',                    s:'minimum',                     t:'n'},
        {c:'maximum',                    s:'maximum',                     t:'n'},
       	{c:'integersOnly',               s:'values.{a}.integers_only',    t:'b'},
        {c:'usePrefixes',                s:'usePrefixes',                 t:'b'}
        ...
        this.PeriodSelectorMap = [  
            ...
            {c:'periodsTitle',                      s:'periods_title',                  t:'s'},
            {c:'customPeriodTitle',                 s:'custom_period_title',            t:'s'}
            ],
            
        this.stockLegendMap = [
            {c:'position',                  s:'position',               t:'s'},
            {c:'align',                     s:'align',                  t:'s'},
            {c:'color',                     s:'value_color',            t:'c'},
            {c:'fontSize',                  s:'text_size',              t:'n'},
            {c:'backgroundColor',           s:'color',                  t:'c'},
            {c:'backgroundAlpha',           s:'alpha',                  t:'a'},
            {c:'borderColor',               s:'border_color',           t:'c'},
...
/* Load JS charts if possible. If the browser doesn't support SVG load Flash charts instead */


    function createChart(type, chart_data, chart_settings, chartContainerID, flashID, flashContent)
    {                 
             if (AmCharts.recommended() == "js")
             {    
                 var chartMapping = [{param:'RADAR', chartType:'radar'},
                                     {param:'LINE', chartType:'line'},
                                     {param:'PIE_2D', chartType:'pie'},
                                     {param:'BAR_VERTICAL', chartType:'column'},
                                     {param:'BAR_HORIZONTAL', chartType:'column'},
                                     {param:'BAR_CLUSTERED', chartType:'column'},
                                     {param:'TIME_GRAPH', chartType:'stock'}];  
                 
                 for (var i = 0; i < chartMapping.length; i++)
                 {
                     var param = chartMapping[i].param;
                     var chartType = chartMapping[i].chartType;
                     
                     if (param == type) 
                     {
                         createJSChart(chartType, chart_data, chart_settings, chartContainerID);
                         break;
                     }
                 }
             ...
             {    
                 createFlashChart(chartContainerID, flashID, flashContent);  
             }               
    }

    function createJSChart(type, chart_data, chart_settings, chartContainerID)
    {                           
              var amFallback = new AmCharts.AmFallback();
              amFallback.chartSettings = chart_settings;
              amFallback.chartData = chart_data;
              amFallback.type = type;
              amFallback.write(chartContainerID);
     }

    function createFlashChart(chartContainerID, flashID, flashContent)
    {
               
             if($(chartContainerID))
             {
                 $(chartContainerID).parentNode.innerHTML = flashContent;
                 if($(flashID+"_flash"))
                 {
                     $(flashID+"_flash").style.visibility = 'visible';
                 }
      ...
      ************** Script for replacing SVGs by images in wiki pages **********************
             ...
             case '[object HTMLDivElement]':
              ...
              default:             
                  ...
                  for ( var i=0;i<images.length;i++) {
                      anything.push(images[i].parentNode);
                  }             
               break;
          }
        return anything;
     ...
     for(var i1=0;i1<chartContainer.length;i1++) {
         ...
         var canvas        = document.createElement('canvas');
          ...
          // Loop through all svgs within the container
              ...
              //replace svg elements by images
              
              var toBeReplaced = chartContainer[i1].getElementsByTagName('svg');
              if(toBeReplaced.length !=0)
              
              chartContainer[i1].replaceChild(image, toBeReplaced[0]); 
